...
function Library:save_flags()
    ...
    writefile(`elysian/{game.GameId}.lua`, flags)
end

function Library:load_flags()
    if not isfile(`elysian/{game.GameId}.lua`) then Library.save_flags() return end

    local flags = readfile(`elysian/{game.GameId}.lua`)
